...
function exportCSV(receipts: Voucher[], payments: Voucher[], openingBalance: number) {
  const rows: string[] = [
  ...
  return (
    <div className="p-6" style={{ fontFamily: "Inter, sans-serif", background: "#F8FAFC", minHeight: "100%" }}>
      ...
      <div className="flex items-center justify-between mb-5">
        <div>
          <h1 className="text-xl font-semibold" style={{ color: "#111827" }}>Cash Book</h1>
          <p className="text-[11px] mt-0.5" style={{ color: "#6B7280" }}>Navkar Freight Co. · All cash transactions</p>
        </div>
        <div className="flex items-center gap-2">
          ...
          <button
            onClick={() => exportCSV(receipts, payments, openingBalance)}
              ...
              <tbody>
                {loading
                  ...
                  : receipts.length === 0
                    ? (
                      <tr>
                        <td colSpan={3} className="px-4 py-8 text-center text-[13px]" style={{ color: "#9CA3AF" }}>No receipts</td>
                      </tr>
                    )
                    : receipts.map((v) => (
                      <tr key={v.id} className="border-b" style={{ borderColor: "#F3F4F6" }}
                        ...
                        <td className="px-4 py-2.5">
                          <div className="font-medium text-[13px]" style={{ color: "#111827" }}>{v.narration ?? "Receipt"}</div>
              ...
              <tbody>
                {loading
                  ...
                  : payments.length === 0
                    ? (
                      <tr>
                        <td colSpan={3} className="px-4 py-8 text-center text-[13px]" style={{ color: "#9CA3AF" }}>No payments</td>
                      </tr>
                    )
                    : payments.map((v) => (
                      <tr key={v.id} className="border-b" style={{ borderColor: "#F3F4F6" }}
                        ...
                        <td className="px-4 py-2.5 text-[12px]" style={{ color: "#6B7280" }}>{v.date?.split("T")[0] ?? ""}</td>
                        <td className="px-4 py-2.5">
                          <div className="font-medium text-[13px]" style={{ color: "#111827" }}>{v.narration ?? "Payment"}</div>
                          <div className="text-[11px] font-mono" style={{ color: "#9CA3AF" }}>{v.voucherNo}</div>
                        </td>
      ...
      {/* Padding for row count parity */}
      {!loading && maxRows === 0 && (
        <div className="mt-4 flex flex-col items-center justify-center py-6">
          <span className="material-symbols-outlined mb-2" style={{ fontSize: 36, color: "#E5E7EB" }}>account_balance_wallet</span>
