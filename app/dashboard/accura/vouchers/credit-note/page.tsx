...
export default function CreditNotePage() {
  ...
  const handleSave = useCallback(async () => {
    if (!partyLedgerId) { alert("Select a customer / party"); return; }
    if (!returnLedgerId) { alert("Select the income/return ledger"); return; }
    if (!baseAmt || baseAmt <= 0) { alert("Enter a valid amount"); return; }

    setSaving(true);
    try {
      // Credit Note: income/return ledger Dr, party (customer) Cr, GST ledgers Dr (reversing GST)
      const apiLines: { ledgerId: string; type: string; amount: number; cgst?: number; sgst?: number; igst?: number; narration?: string; }[] = [];

      // Return/income ledger as Dr (base amount — reversing the income)
      apiLines.push({ ledgerId: returnLedgerId, type: "Dr", amount: baseAmt, cgst, sgst, igst });
      ...
      if (supplyType === "intra" && cgst > 0) {
        ...
        if (cgstLedger) apiLines.push({ ledgerId: cgstLedger.id, type: "Dr", amount: cgst });
        if (sgstLedger) apiLines.push({ ledgerId: sgstLedger.id, type: "Dr", amount: sgst });
      } else if (supplyType === "inter" && igst > 0) {
        const igstLedger = ledgerOptions.find((l) => l.name.toLowerCase().includes("igst"));
        if (igstLedger) apiLines.push({ ledgerId: igstLedger.id, type: "Dr", amount: igst });
      }

      // Party (customer) as Cr for total credit
      apiLines.push({ ledgerId: partyLedgerId, type: "Cr", amount: total, narration: originalInvoice ? `Ref: ${originalInvoice}` : undefined });
  ...
  const customerOpts = ledgerOptions.length > 0
    ? ledgerOptions
        .filter((l) => ["Sundry Debtors"].includes(l.group?.name ?? ""))
        .map((l) => ({ id: l.id, label: l.name, sub: l.group?.name ?? "" }))
    : [
        { id: "__ravi", label: "Ravi Exports Pvt Ltd", sub: "Sundry Debtors" },
        { id: "__hdfc_t", label: "HDFC Traders", sub: "Sundry Debtors" },
        ...
        <div className="p-6 space-y-5">
          <div className="grid grid-cols-3 gap-4">
            ...
            <div>
              ...
              <input type="text" placeholder="INV/25-26/..." value={originalInvoice} onChange={(e) => setOriginalInvoice(e.target.value)} className="w-full px-3 py-2 rounded-md border text-[13px] outline-none focus:border-[#0E7490]" style={{ borderColor: "#E5E7EB", color: "#111827" }} />
            </div>
          </div>

          <TypeAheadInput label="Customer / Party (Cr) *" placeholder="Select customer..." options={customerOpts} value={partyLedgerId} onChange={setPartyLedgerId} required />
          <TypeAheadInput label="Income / Return Ledger (Dr) *" placeholder="Ledger to reduce..." options={incomeOpts} value={returnLedgerId} onChange={setReturnLedgerId} required />
          ...
          <div className="grid grid-cols-3 gap-4">
            <div>
              ...
              <div className="relative">
                ...
                <input type="number" placeholder="0.00" value={amount} onChange={(e) => setAmount(e.target.value)} className="w-full pl-7 pr-3 py-2 rounded-md border text-[13px] text-right outline-none focus:border-[#0E7490]" style={{ borderColor: "#E5E7EB", color: "#111827" }} />
              </div>
            </div>
            <div>
            ...
            <div className="rounded-md p-3 border" style={{ borderColor: "#A5F3FC", background: "#ECFEFF" }}>
              <div className="text-[11px] mb-1" style={{ color: "#0E7490" }}>
                GST: ₹{gstAmt.toLocaleString("en-IN", { minimumFractionDigits: 2 })}
              </div>
              <div className="text-base font-bold" style={{ color: "#111827" }}>
                Total Credit: ₹{total.toLocaleString("en-IN", { minimumFractionDigits: 2 })}
              </div>
            </div>
          </div>
          ...
          <div>
            ...
            <textarea rows={2} placeholder="Sales return / discount given..." value={reason} onChange={(e) => setReason(e.target.value)} className="w-full px-3 py-2 rounded-md border text-[13px] outline-none resize-none focus:border-[#0E7490]" style={{ borderColor: "#E5E7EB", color: "#111827" }} />
          </div>
        </div>
        ...
        <div className="flex items-center justify-between px-5 py-3 border-t rounded-b-xl" style={{ borderColor: "#F3F4F6", background: "#F9FAFB" }}>
          ...
          <div className="flex items-center gap-2">
            ...
            >
              ...
              {saving ? "Saving…" : saved ? "Saved ✓" : "Save Credit Note (Ctrl+A)"}
            </button>
          </div>
        </div>
      </motion.div>
