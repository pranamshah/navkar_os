...
type FollowUp = { name: string; jobNo: string; overdue: string };
const followUps: FollowUp[] = [];

export default function EntryXDashboard() {
  return (
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-xs mt-0.5" style={{ color: "#6B7280" }}>
            Navkar Freight Co. · Customs House Agent · FY 2025–26
          </p>
        </div>
      ...
      <div className="grid grid-cols-3 gap-4 mb-6">
        <div className="col-span-2 rounded-xl border p-5" style={{ background: "#fff", borderColor: "#E5E7EB" }}>
          <div className="flex items-center justify-between mb-4">
            ...
            <Link href="/dashboard/entryx/jobs" className="text-[11px] font-medium" style={{ color: ACCENT }}>
              View all →
            </Link>
          </div>
          <table className="w-full text-[12px]">
            <thead>
              <tr style={{ borderBottom: "1px solid #E5E7EB" }}>
                {["Job No", "Client", "Type", "Mode", "HS Code", "Stage", "BE No", ""].map((h) => (
                  <th
                    key={h}
            ...
            <tbody>
              {recentJobs.length === 0 ? (
                <tr>
                  <td colSpan={8} className="py-10 text-center text-[12px]" style={{ color: "#6B7280" }}>
                    No jobs yet.
                  </td>
                </tr>
              ) : (
                recentJobs.map((j) => (
                  <tr key={j.no} style={{ borderBottom: "1px solid #F3F4F6" }}>
                    <td className="py-2.5 px-2 font-mono text-[11px]" style={{ color: ACCENT }}>{j.no}</td>
                    ...
                    <td className="py-2.5 px-2 font-mono text-[11px]" style={{ color: "#6B7280" }}>{j.hsCode}</td>
                    <td className="py-2.5 px-2">
                      <span
                        className="px-2 py-0.5 rounded-full text-[10px] font-bold"
                        style={{ background: j.stageBg, color: j.stageColor }}
                      >
                        {j.stage}
                      </span>
                    </td>
                    <td className="py-2.5 px-2 font-mono text-[11px]" style={{ color: "#6B7280" }}>{j.beNo || "—"}</td>
        ...
        <div className="rounded-xl border p-4" style={{ background: "#fff", borderColor: "#E5E7EB" }}>
          <h2 className="text-sm font-semibold mb-3" style={{ color: "#111827" }}>Compliance Alerts</h2>
          <div className="flex flex-col gap-2">
            {alerts.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-8 text-center">
                ...
                <p className="text-[12px]" style={{ color: "#7e7576" }}>No compliance alerts.</p>
              </div>
            ) : (
              alerts.map((a, i) => (
                <div
      ...
      <div className="rounded-xl border p-4" style={{ background: "#fff", borderColor: "#E5E7EB" }}>
        ...
        {followUps.length === 0 ? (
          ...
          <div className="grid grid-cols-3 gap-3">
            {followUps.map((f) => (
              <div key={f.jobNo} className="p-3 rounded-lg border" style={{ borderColor: "#FECACA", background: "#FEF2F2" }}>
                <div className="text-[13px] font-semibold" style={{ color: "#111827" }}>{f.name}</div>
                <div className="text-[11px] mt-0.5 font-mono" style={{ color: "#6B7280" }}>{f.jobNo}</div>
